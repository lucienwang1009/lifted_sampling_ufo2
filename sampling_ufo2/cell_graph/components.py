...
class BtypeTable(object):
    ...
    def get_weight(self, evidences: FrozenSet[Lit] = None) -> np.ndarray:
        ...
        return np.sum(df.weight, axis=0)

    @functools.lru_cache(maxsize=None)
    def get_btypes(self, evidences: FrozenSet[Lit] = None) -> Tuple[FrozenSet[Lit], np.ndarray]:
        if not self.satisfiable(evidences):
            logger.warning(
                'Cell pair (%s, %s) with evidences %s is not satisfiable',
                self.cell_1, self.cell_2, evidences
